...
Files the issue,
optionally spawns a Codex agent in a fresh worktree, and lets /ship close
...
~/.Codex/skills/gstack/bin/gstack-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.Codex/skills/gstack/bin/gstack-question-log '{"skill":"spec","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
| `--execute` | conditional default (see Phase 5) | Spawn `Codex -p` in a fresh worktree after filing the issue. |
...
bun ~/.Codex/skills/gstack/lib/redact-audit-log.ts \
...
REDACT_VIS=$(~/.Codex/skills/gstack/bin/gstack-config get redact_repo_visibility 2>/dev/null)
...
REDACT_BODY_EOF
REDACT_JSON=$(~/.Codex/skills/gstack/bin/gstack-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json)
REDACT_CODE=$?
```
...
reuse it; write the exact bytes to `$REDACT_FILE`; `~/.Codex/skills/gstack/bin/gstack-redact --from-file "$REDACT_FILE"
...
eval "$(~/.Codex/skills/gstack/bin/gstack-slug)"
...
If A and worktree created: spawn `Codex -p` with the spec piped via stdin:

```bash
cat "$ARCHIVE_PATH" | (cd "$SPAWN_PATH" && Codex -p 2>&1) &
SPAWN_PID=$!
echo "Spawned: PID $SPAWN_PID in $SPAWN_PATH (branch $SPAWN_BRANCH)"
echo "Follow with: cd $SPAWN_PATH && Codex --resume"
```
