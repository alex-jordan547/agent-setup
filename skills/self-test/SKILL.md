...
- **Name the proof artifacts you'll create when the surface is visual or interactive.** Screenshots prove final visible state; short screen recordings prove multi-step flows, focus behavior, animation, scrolling, or timing. Store them under a durable path such as `~/.codex/proofs/<thread-or-task>/<slug>/`.
...
- Attach or link durable proof artifacts for visual and interaction checks. In Codex Desktop, reference local artifacts with absolute paths, for example `![final state](~/.codex/proofs/<task>/final.png)` or `[flow recording](~/.codex/proofs/<task>/flow.mov)`.
